...
member's to fix, and replacing it would silently delete their other tools.
    ...
    python3 scripts/register_mcp_client.py --command /path/to/TelescopeNetNode
    python3 scripts/register_mcp_client.py --remove
"""
...
def entry(command: str, data_dir: str) -> dict:
    args = ["--mcp"]
    if data_dir:
...
def register(command: str, data_dir: str, path: Path) -> tuple[bool, str]:
    ...
    desired = entry(command, data_dir)

...
class TonightLocalDefaultTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server = MCPServer(name="t", title="t", instructions="t", version="0")
        tonight.register(self.server, self.client, self.agent)
    ...
    def test_remote_mcp_without_agent_refuses_a_blank_id(self):
        from mcp.server import MCPServer
        server = MCPServer(name="t", title="t", instructions="t", version="0")
        tonight.register(server, self.client, None)
        self.client.post.reset_mock()
        text, _ = _call(server, "tonight_accept", {})
        self.assertIn("telescope computer", text.lower() + " telescope computer")

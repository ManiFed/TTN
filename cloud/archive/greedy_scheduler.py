#!/usr/bin/env python3
"""
Archived greedy nightly plan generation.

This was the pre-CHORUS per-node slot packer.  It is intentionally no longer
imported by the live scheduler dispatch path; keep it here for reference,
historical debugging, and explicit experiments.

Turns scored targets into a concrete observation
...
logger = logging.getLogger("cloud.archive.greedy_scheduler")
...
def generate_all_plans(config: dict) -> int:
    """Generate a fresh archived-greedy plan for every online node."""
    count = 0

...
| # | Stage | Code | Notes |
|---|-------|------|-------|
| 1 | Installer writes config | `build/macos/postinstall.sh`, `build/linux/install.sh`, `build/windows/install.nsi` | Template copied to data dir verbatim, `chmod 600`. Nothing is substituted — there are no activation codes. |
...
| 5 | Cloud registration | `cloud_communicator._ensure_registered` → `POST /api/v1/nodes/register` | Credentials arrive from the app (`POST /api/cloud/credentials`) or via pair-token polling (`_pair_loop`); self-registration is the fallback. Persisted to `data/cloud_state.json`. |
...
|----|-------------|---------|------------------|-----|
| **F7. Activation bricked by lost registration response** — *retired July 2026* | Was: server consumed a one-time activation code, the response was lost, and the node retried forever against a used code | No longer reachable. Activation codes are gone; linking is idempotent (`POST /me/nodes/attach` reuses a never-online node instead of minting a duplicate). | Closed. |
...
| **F11. Registration failure loop is invisible** | Bad URL or cloud outage at first boot | `_ensure_registered` retries every 60 s, logs a warning locally. Dashboard shows error *if someone opens it*. Cloud can't see a node it doesn't know. | Installer/pair flow has no "phone home failed" surface; no backoff; pair token only printed to stdout. |
...
| **Registration resilience** — exponential backoff (60 s → 15 min) with `registration_failed` events; attach reuses a member's never-heartbeated node instead of minting a duplicate (lost-response recovery) | F7, F11 | `cloud_communicator._ensure_registered`, `cloud/server.py api_me_attach_node` |
